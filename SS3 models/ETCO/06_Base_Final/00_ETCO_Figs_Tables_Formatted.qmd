...
refpoints.ft <- compose(refpoints.ft, i = 2, j = 1, part = "body", 
                        value = as_paragraph(as_i("F"), as_sub("2025"), " (yr", as_sup("-1"), ")"))

refpoints.ft <- compose(refpoints.ft, i = 3, j = 1, part = "body", 
                        value = as_paragraph(as_i("F"), as_sub("2025"), "/", as_i("F"), as_sub("MSY")))
...
refpoints.ft <- compose(refpoints.ft, i = 5, j = 1, part = "body", 
                        value = as_paragraph(as_i("SSB"), as_sub("2025"), " (mt)"))
refpoints.ft <- compose(refpoints.ft, i = 6, j = 1, part = "body", 
                        value = as_paragraph(as_i("SSB"), as_sub("2025"), "/", as_i("SSB"), as_sub("MSST")))
...
refpoints.ft <- compose(refpoints.ft, i = 8, j = 1, part = "body", 
                        value = as_paragraph("Catch", as_sub("2023-2025"), " (mt)"))
...
refpoints.ft <- compose(refpoints.ft, i = 10, j = 1, part = "body", 
                        value = as_paragraph(as_i("SPR"), as_sub("2025")))
...
```{r}
#| label: fig-kobe
#| fig-cap: "Kobe plot representing the trend in relative fishing mortality and spawning stock biomass between 1969 and 2025 with their associated biological reference areas (red: overfished and overfishing, yellow: overfishing or overfished, green: no overfishing and not overfished). The large red dot indicates median stock status in 2025 and the black dots are one thousand Monte Carlo draws from the stock status distribution to represent the uncertainty around the final year status."
#| out-width: 14cm
#| fig-align: center
...
Results.ft <- compose(Results.ft, j = 2, part = "header", 
                        value = as_paragraph(as_i("F"), as_sub("2025")))
...
Results.ft <- compose(Results.ft, j = 7, part = "header", 
                        value = as_paragraph(as_i("SSB"), as_sub("2025")))
Results.ft <- compose(Results.ft, j = 8, part = "header", 
                        value = as_paragraph(as_i("SSB"), as_sub("2025"), "/", as_i("SSB"), as_sub("MSY")))
...
```{r}
#| tbl-cap: "The annual fixed catch values (metric tons) applied from 2027 to a final projection year resulting in a given probability of overfishing (F/FMSY>1) in that final year. Catch for the year prior to the start of the new catch guidance (2026) was fixed at the mean of the last 3 years of catch data (2023 to 2025)."
...
proj.table.ft <- add_header_row(proj.table.ft, 
                                values = c("", "Fixed catch (mt) from 2027 to:"), colwidths = c(1, 5))
...
#| fig-cap: "Probability of overfishing (left panel) and of stock being overfished (right panel) for a range of catch values (metric tons) fixed for a given range of years starting in 2028 The stock status probabilities are for the final projection year."
#| fig-align: center
